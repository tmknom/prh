...
LABEL org.label-schema.vendor="tmknom" \
      ...
      org.label-schema.description="A proofreading helper." \
      ...
      org.label-schema.docker.cmd="docker run --rm -v \$PWD:/work $REPO_NAME --rules prh.yml ./*.md" \
      org.label-schema.schema-version="1.0"

ARG NODEJS_VERSION=10.14.2-r0
ARG MODULE_VERSION=5.4.3

RUN set -x && \
    apk add --no-cache nodejs=${NODEJS_VERSION} nodejs-npm=${NODEJS_VERSION} && \
    npm install -g prh@${MODULE_VERSION} && \
    npm cache clean --force && \
    apk del nodejs-npm

WORKDIR /work
ENTRYPOINT ["/usr/bin/prh"]
CMD ["--help"]
